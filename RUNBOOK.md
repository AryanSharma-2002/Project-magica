...
pnpm test:watch
E2E_CLERK_USER_EMAIL=<reviewer email> pnpm test:e2e   # Playwright smoke against the running stack (sections 6, 9b; e2e/README.md); spends one OpenRouter request
```
...
Locally, restart both `pnpm dev` and `pnpm trigger:dev` after adding these variables: neither process picks up new `.env` keys while running.
