...
6. **One transaction**: reserve admission (`reserve:<runId>`, `ADMISSION_MICROCREDITS`; `insufficient_credits` if balance too low) → create user `Message(COMPLETED)` + assistant `Message(PENDING, content: [])` → create `AgentRun(QUEUED)` → link attachments to the user message → bump `Chat.lastMessageAt` (+ title from first message). The partial unique index is the race arbiter; a unique violation on it maps to `run_active`.
