...
function toProviderInput(input: MergeVideosInput): Record<string, unknown> {
...
function normalizeOutput(raw: unknown): MergeVideosOutput {
    ...
    estimateWithFallback({
      ...
      fallback: () =>
        catalogCostFallback({
          ...
          log: ctx.log,
          pick: (cost) => {
            const perMinute = pickNumberField(cost, "perMinute", "per_minute");
            if (perMinute === undefined) return undefined;
            const extraPerItem = pickNumberField(cost, "extraPerItem", "extra_per_item") ?? 0;
            return perMinute + extraPerItem * Math.max(0, input.video_urls.length - 1);
          },
        }),
    }),
